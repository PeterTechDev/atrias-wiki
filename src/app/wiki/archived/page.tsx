...
export default function ArchivedPages() {
  ...
  async function remove() {
    ...
    try {
      ...
      if (!response.ok) setDeleteError((await response.json()).error || 'Não foi possível excluir.')
  ...
  return <main className="min-h-screen bg-zinc-900 px-4 py-8 text-zinc-100 sm:px-8">
    <div className="mx-auto max-w-4xl"><Link href="/" className="text-amber-400">← Voltar</Link><h1 className="mt-4 text-3xl font-bold text-amber-400">Páginas arquivadas</h1>
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3"><label className="flex items-center gap-2"><input type="checkbox" checked={allSelected} onChange={() => setSelected(allSelected ? [] : entities.map((entity) => entity.id))} /> Selecionar todos ({entities.length})</label><button disabled={!selected.length || busy} onClick={() => { setDeleteError(''); setConfirming(true) }} className="rounded bg-red-800 px-4 py-2 disabled:opacity-40">Excluir selecionados ({selected.length})</button></div>
    ...
    <dialog ref={deleteDialog} onCancel={() => setConfirming(false)} aria-labelledby="delete-title" className="max-w-lg rounded-lg bg-slate-900 p-6 text-slate-100 backdrop:bg-black/70"><h2 id="delete-title" className="text-xl font-semibold">Excluir definitivamente?</h2><p className="mt-3">A exclusão permanente remove {selected.length} página(s) e não pode ser desfeita.</p><ul className="mt-3 list-disc pl-5 text-sm">{names.map((name) => <li key={name}>{name}</li>)}</ul>{deleteError && <p role="alert" className="mt-3 text-red-300">{deleteError}</p>}<div className="mt-6 flex justify-end gap-2"><button onClick={() => { setConfirming(false); deleteDialog.current?.close() }} disabled={busy} className="rounded border border-slate-500 px-4 py-2">Cancelar</button><button onClick={remove} disabled={busy} className="rounded bg-red-800 px-4 py-2">{busy ? 'Excluindo…' : 'Excluir definitivamente'}</button></div></dialog>
  </main>
}
