...
export default function ArchivedPages() {
  ...
  return <main className="min-h-screen bg-zinc-900 px-4 py-8 text-zinc-100 sm:px-8">
    ...
    {confirming && <div role="dialog" aria-modal="true" aria-labelledby="delete-title" className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4"><div className="max-w-lg rounded-lg bg-slate-900 p-6"><h2 id="delete-title" className="text-xl font-semibold">Excluir definitivamente?</h2><p className="mt-3">A exclusão permanente remove {selected.length} página(s) e não pode ser desfeita.</p><ul className="mt-3 list-disc pl-5 text-sm">{names.map((name) => <li key={name}>{name}</li>)}</ul><div className="mt-6 flex justify-end gap-2"><button onClick={() => setConfirming(false)} disabled={busy} className="rounded border border-slate-500 px-4 py-2">Cancelar</button><button onClick={remove} disabled={busy} className="rounded bg-red-800 px-4 py-2">{busy ? 'Excluindo…' : 'Excluir definitivamente'}</button></div></div></div>}
  </main>
}
