...
export default function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    function close(event: MouseEvent) { if (menuRef.current && !menuRef.current.contains(event.target as Node)) setMenuOpen(false) }
    document.addEventListener('mousedown', close)
    return () => document.removeEventListener('mousedown', close)
  }, [])

  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) { if (event.key === 'Escape' && menuOpen) { setMenuOpen(false); triggerRef.current?.focus() } }
  ...
  return <AuthContext.Provider value={{ user, loading }}>
    <nav aria-label="Conta" className="relative z-40 flex min-h-16 flex-wrap items-center justify-end gap-3 border-b border-amber-200/10 bg-[#0a1628] px-4 py-2 text-sm text-amber-100 sm:px-6">
      {loading ? <span role="status">Carregando conta…</span> : user ? <>
        ...
        <button ref={triggerRef} type="button" aria-expanded={menuOpen} aria-controls="account-menu" onClick={() => setMenuOpen(value => !value)} onKeyDown={(event) => { if (event.key === 'Escape') { setMenuOpen(false); triggerRef.current?.focus() } }} className="flex min-w-0 items-center gap-2 rounded focus-visible:outline-2 focus-visible:outline-amber-400">
          <Avatar value={user.user_metadata.avatar} />
          <span className="max-w-40 truncate">{user.user_metadata.display_name || 'Meu perfil'}</span>
        </button>
        {menuOpen && <div id="account-menu" className="absolute right-0 top-full z-50 mt-2 w-44 rounded border border-amber-200/30 bg-[#0a1628] p-1 shadow-xl"><Link href="/login" onClick={() => setMenuOpen(false)} className="block rounded px-3 py-2 hover:bg-amber-100/10">Meu perfil</Link><Link href="/wiki/favorites" onClick={() => setMenuOpen(false)} className="block rounded px-3 py-2 hover:bg-amber-100/10">Favoritos</Link></div>}
        </div>
